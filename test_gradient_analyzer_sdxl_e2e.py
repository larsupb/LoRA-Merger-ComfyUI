#!/usr/bin/env python3
"""
End-to-End Test for PM LoRA Semantic Analyzer (Gradient) - SDXL Version.

This test replicates the COMPLETE workflow from PM-Gradient-Analyzer-SDXL.json:
1. Load Checkpoint (SDXL model with UNet, CLIP, VAE)
2. Stack LoRAs
...
Based on: /home/lars/SD/Apps/ComfyUI/user/default/workflows/PM-Gradient-Analyzer-SDXL.json
"""
...
print("=" * 80)
print("PM LoRA Semantic Analyzer (Gradient) - SDXL END-TO-END TEST")
print("=" * 80)
...
try:
    ...
    from src.nodes_semantic_merge import (
        PMLoRASemanticAnalyzerGradient,
        PMSemanticMergeSpec,
        PMSemanticMerger,
    )
...
# Test configuration from workflow (PM-Gradient-Analyzer-SDXL.json)
CONFIG = {
    # Checkpoint (from node 135)
    "checkpoint": "SDXL/realismByStableYogi_v60FP16.safetensors",

    # LoRAs (from node 105)
    "loras": [
        {"on": True, "lora": "Pony/Characters/SarahF/SarahF-Pony-LoRA.safetensors", "strength": 1.0},
        {"on": True, "lora": "Pony/Characters/SDXL-SpencerN-Pony-LoRA.safetensors", "strength": 1.0},
    ],
    "layer_filter": "attn-mlp",

    # Gradient analysis (from node 134)
    "checkpoint_name": "checkpoints/SDXL/realismByStableYogi_v60FP16.safetensors",
    "features": "face,breasts",
    "architecture_hint": "sdxl",
    "num_samples": 3,  # From workflow
    "use_cache": False,  # From workflow
    "device": "auto",
    "dtype": "float8_e4m3fn",  # FP8 for memory efficiency
    "offload_layers": True,
    "gpu_memory_gb": 0.0,  # 0 = auto-detect 80% of VRAM

    # Merge spec (from node 113)
    "merge_spec": "face from lora1, breasts from lora2",

    # Prompts (from nodes 6 and 7)
    # Note: Workflow has empty positive prompt, using generic SDXL prompt instead
    "positive_prompt": """A photorealistic portrait of a person, professional photography,
high detail, sharp focus, natural lighting, 8k uhd, masterpiece, best quality""",
    ...
    # Output
    "output_image": "test_output_gradient_analyzer_sdxl.png",
}
...
def main():
    ...
    try:
        # Step 1: Load Checkpoint (SDXL model with UNet, CLIP, VAE)
        current_step += 1
        test_step(current_step, total_steps, "Load SDXL Checkpoint")

        checkpoint_loader = nodes.CheckpointLoaderSimple()
        model, clip, vae = checkpoint_loader.load_checkpoint(CONFIG["checkpoint"])
        print(f"✓ SDXL checkpoint loaded: {CONFIG['checkpoint']}")
        ...
        current_step += 1
        test_step(current_step, total_steps, "Gradient-Based Semantic Analysis (SDXL)")
        ...
        print()
        print("  ⚠ This may take 2-10 minutes on first run (downloads SDXL config + analysis)")
        ...
        print()
        print(f"Architecture: SDXL (UNet-based, 2.6B params)")
        ...
        print(f"  GPU memory limit: {CONFIG['gpu_memory_gb']} GB (0=auto)")
        print(f"  Data type: {CONFIG['dtype']}")
        print()
        print("🎉 SDXL END-TO-END TEST PASSED!")
        print()
        print("Note: This test demonstrates gradient-based semantic analysis on SDXL,")
        print("which works on 8GB VRAM GPUs with layer offloading enabled.")
